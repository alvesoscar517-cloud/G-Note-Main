...
async function callAI(endpoint: string, body: Record<string, string>): Promise<string> {
  ...
  // Handle errors (non-200 responses that might be JSON)
  if (!response.ok) {
    let errorData;
    try {
    ...
    // Handle insufficient credits specific code
    ...
    for (const line of lines) {
      if (line.startsWith('data: ')) {
        try {
          ...
          // Ignore parse errors for partial lines or if it's not JSON
          if (e instanceof Error && e.message !== "Unexpected end of JSON input") {
            // console.warn('Error parsing AI stream chunk:', e)
          }
        }
      }
    }
